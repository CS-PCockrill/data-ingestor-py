...
class OracleQueryBuilder(QueryBuilder):
    ...
    def build_insert_query_oracle(self, columns, batch=True):
        """
        ...
        Args:
            ...
            batch (bool): If True, uses named placeholders for batch inserts.
        ...
        if batch:
            # Oracle does not support "%s", so use named placeholders like :1, :2, ...
            values_placeholder = f"({', '.join([f':{i + 1}' for i in range(len(columns))])})"
        else:
            # Single insert, still use positional placeholders
            values_placeholder = f"({', '.join([f':{i + 1}' for i in range(len(columns))])})"

        # Generate the query with RETURNING clause for Oracle
        return f"INSERT INTO {self.table_name} ({col_list}) VALUES {values_placeholder} RETURNING id INTO :id;"
